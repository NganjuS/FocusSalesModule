...
function initRmaSearch(response) {
    ...
    Focus8WAPI.getFieldValue("getRma", ["", "DocNo", "RmaSearch"], Focus8WAPI.ENUMS.MODULE_TYPE.TRANSACTION, false, requestId);
}
function getRma() {
    ...
    script.onload = () => {
        ...
        openPopup(companyid, vtype, docno, rmano);
    };
...
openPopup(companyid, vtype, docno, rmano)
{
    try
    {
        ...
        let stemUrl = `/subatiintegrations/api/masters/varietystocks/?compid=${compid}&varietyid=${varietyid}&txndate=${docDate}`;
        let response = await fetch(stemUrl);
        ...
        if (dataObj.result == -1) {
            ...
            alert("Unable to load stem lengths: " + err);
