...
function InitOpenPos(response) {
    ++requestId;
    Focus8WAPI.getFieldValue("openPOS", ["", "DocNo","Outlet"], Focus8WAPI.ENUMS.MODULE_TYPE.TRANSACTION, false, requestId);

}
function InitOpenPosAfterSave(response) {
    ++requestId;
    Focus8WAPI.getFieldValue("openPOS", ["", "DocNo", "Outlet"], Focus8WAPI.ENUMS.MODULE_TYPE.TRANSACTION, false, requestId);
...
function openPOS(response) {
    ...
    var outletId = response.data[2].FieldValue;
    var companyid = response.data[0].CompanyId;
    var sessid = response.data[0].SessionId;
    var vtype = response.data[0].iVoucherType;

    Focus8WAPI.openPopup(`/focussalesmodule/posscreen/openpos/?compid=${companyid}&vtype=${vtype}&outletid=${outletId}&sessionid=${sessid}&docno=${docNo}`, null);
