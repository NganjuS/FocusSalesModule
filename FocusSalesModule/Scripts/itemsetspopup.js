...
function initItemSets(response) {

    ++requestId;
    Focus8WAPI.getFieldValue("openPopup", ["", "DocNo"], Focus8WAPI.ENUMS.MODULE_TYPE.TRANSACTION, false, requestId);
}
function openPopup(response) {
    if (isRequestProcessed(response.iRequestId)) {
        return;
    }
    let validRows = response.data[0].RowsInfo.iValidRows;

    if (validRows > 0) {

        alert("Create new transaction to continue !!!");
        return
    }
    ...
    let url = `/focussalesmodule/PreOrderSales/ItemSets/?compid=${response.data[0].CompanyId}`;
    Focus8WAPI.openPopup(url, "afterPopupOpen");
   

    //Focus8WAPI.continueModule(, true)
}


function afterPopupOpen(response) {
