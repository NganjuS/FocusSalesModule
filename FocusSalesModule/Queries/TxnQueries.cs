...
namespace FocusSalesModule.Queries
{
    public class TxnQueries
    {
        ...
        {
            return $"select ptyp.sName PaymentType,putyp.* from muPos_Outlet_PaymentType_Details pdet join mCore_paymenttype ptyp on ptyp.iMasterId = pdet.TypeOfPayment  join  muCore_paymenttype putyp on putyp.iMasterId = ptyp.iMasterId  where pdet.iMasterId = {outletId} and pdet.TypeOfPayment <> 0 ";
        }
        public static string GetPOSData(int outletId, int dateFrom, int dateTo)
        {
            return $"select  h.iHeaderId HeaderId,h.sVoucherNo DocNo, dbo.IntToDate(h.iDate) DocDate,cust.iMasterId CustAccId ,cust.sName CustomerName,mem.iMasterId MemberId, mem.sName MemberName,outl.iMasterId OutletId, outl.sName OutletName,cst.iMasterId CostCenterId, cst.sName CostCenterName, abs(sum(stk.fQuantity)) Quantity,  abs(sum(stk.mGross)) GrossAmt,abs(max(h.fNet)) Amount,he.sNarration Narration, usr.sLoginName CreatedBy from tCore_Header_0 h join tCore_HeaderData3335_0 he on he.iHeaderId = h.iHeaderId join tCore_Data_0 d on d.iHeaderId = h.iHeaderId join tCore_Indta_0 stk on stk.iBodyId = d.iBodyId join mCore_Product pr on pr.iMasterId = stk.iProduct  left join mCore_Account cust on cust.iMasterId = d.iBookNo left join tCore_Data_Tags_0 tg on tg.iBodyId = d.iBodyId left join mPos_Outlet outl on outl.iMasterId = d.iInvTag  left join mCore_CostCenter cst on cst.iMasterId = d.iFaTag  left join mSec_Users usr on usr.iUserId = h.iUserId left join  mPos_Member mem on mem.iMasterId = tg.iTag1106  where h.iVoucherType = 3335  and h.idate >= {dateFrom} and h.idate <= '{dateTo}' and outl.iMasterId = {outletId}  group by h.iHeaderId ,h.sVoucherNo , h.iDate,cust.iMasterId  ,cust.sName ,mem.iMasterId , mem.sName ,outl.iMasterId , outl.sName ,cst.iMasterId , cst.sName, usr.sLoginName, he.sNarration";
        }
        public static string GetPOSDataByHeaderId(int headerId)
        {
            return $"select  iif(h.bSuspended <> 0, 4, iif(h.iAuth < 2, 2, iif(h.iAuth=3, 1, 3)) )DocStatus,h.iHeaderId HeaderId,h.sVoucherNo DocNo, dbo.IntToDate(h.iDate) DocDate,cust.iMasterId CustAccId ,cust.sName CustomerName,mem.iMasterId MemberId, mem.sName MemberName,outl.iMasterId OutletId, outl.sName OutletName,cst.iMasterId CostCenterId, cst.sName CostCenterName, abs(sum(stk.fQuantity)) Quantity,  abs(sum(stk.mGross)) GrossAmt,abs(max(h.fNet)) Amount,he.sNarration Narration, usr.sLoginName CashierName from tCore_Header_0 h join tCore_HeaderData3335_0 he on he.iHeaderId = h.iHeaderId join tCore_Data_0 d on d.iHeaderId = h.iHeaderId join tCore_Indta_0 stk on stk.iBodyId = d.iBodyId join mCore_Product pr on pr.iMasterId = stk.iProduct  left join mCore_Account cust on cust.iMasterId = d.iBookNo left join tCore_Data_Tags_0 tg on tg.iBodyId = d.iBodyId left join mPos_Outlet outl on outl.iMasterId = d.iInvTag  left join mCore_CostCenter cst on cst.iMasterId = d.iFaTag  left join mSec_Users usr on usr.iUserId = h.iUserId left join  mPos_Member mem on mem.iMasterId = tg.iTag1106  where h.iVoucherType = 3335  and h.iHeaderId= {headerId} group by h.iHeaderId ,h.sVoucherNo , h.iDate,cust.iMasterId  ,cust.sName ,mem.iMasterId , mem.sName ,outl.iMasterId , outl.sName ,cst.iMasterId , cst.sName, usr.sLoginName, he.sNarration, h.bSuspended, h.iAuth";
        }
