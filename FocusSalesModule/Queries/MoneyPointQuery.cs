...
namespace FocusSalesModule.Queries
{
    public class MoneyPointQuery
    {
        ...
        {
            return "IF OBJECT_ID(N'MoniePointData', N'U') IS NULL create table MoniePointData\r\n(\r\n  Id int IDENTITY PRIMARY KEY, WebhookId varchar(200),WebhookTimestamp Datetime , Amount DECIMAL(18,4), BusinessId int,     ResponseCode varchar(100), TerminalSerial varchar(100), BusinessOwnerId int , ResponseMessage varchar(100),TransactionTime Datetime,TransactionType varchar(100), MerchantReference varchar(100),  TransactionStatus varchar(100),\r\nTransactionReference varchar(100),RetrievalReferenceNumber  varchar(100),InvoiceID varchar(100), EventId varchar(100),Domain varchar(100),ReqResource varchar(100), ResourceId varchar(100),CreatedAt Datetime,EventType varchar(100), IsAllocatedToSale bit, AmountInLocalCur DECIMAL(18,4), IsValidTxn bit )";
        }
        public static string InsertTransactionsTable(MoniePointRequestDTO requestDTO, string webhookid, string timestamp, bool isValidTxn)
        {
            ...
            decimal amtInLocalCur = requestDTO.data.amount > 0 ?  requestDTO.data.amount / 100 : requestDTO.data.amount;

            return $"insert into MoniePointData (WebhookId ,WebhookTimestamp  , Amount,AmountInLocalCur , BusinessId,     ResponseCode , TerminalSerial , BusinessOwnerId , ResponseMessage ,TransactionTime ,TransactionType , MerchantReference ,  TransactionStatus ,\r\nTransactionReference ,RetrievalReferenceNumber  ,InvoiceID , EventId,Domain ,ReqResource , ResourceId ,CreatedAt ,EventType, IsAllocatedToSale, IsValidTxn ) values ('{webhookid}','{timestamp}',{requestDTO.data.amount},{amtInLocalCur}, {requestDTO.data.businessId},'{requestDTO.data.responseCode}','{requestDTO.data.terminalSerial}',{requestDTO.data.businessOwnerId},'{requestDTO.data.responseMessage}', '{txnTimeSqlFormat}','{requestDTO.data.transactionType}','{requestDTO.data.merchantReference}','{requestDTO.data.transactionStatus}','{requestDTO.data.transactionReference}','{requestDTO.data.retrievalReferenceNumber}','{requestDTO.data.customFields.InvoiceID}','{requestDTO.eventId}','{requestDTO.subject.domain}','{requestDTO.subject.resource}','{requestDTO.subject.resourceId}','{createdAtSqlFormat}','{requestDTO.eventType}',0,{ConvBoolToInt(isValidTxn)})";
        }
        static int ConvBoolToInt(bool val) => val ? 1 : 0;
    }
}